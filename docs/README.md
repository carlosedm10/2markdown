...
- **One local model resident, not two** — page transcription and figure captions used different local models, on the assumption that both could stay loaded. On a 48 GB machine that is 29.1 GB plus 8.8 GB against the ~36 GB macOS leaves the GPU, and the image encoder is what runs out: Ollama logs `kIOGPUCommandBufferCallbackErrorOutOfMemory` inside `clip_encode` and answers with a malformed completion (empty `role`) that pydantic-ai rejects. `effective_figure_model()` reuses the page model whenever both are local — it is already loaded, so a caption costs inference only. A hosted captioner is left alone; it does not compete for the GPU. Measured: 4 failures in 6 pages before, 6/6 after.
